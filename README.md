...
MindHub는 환자가 진료실 밖에서 남긴 일상 대화를 바탕으로 수면·기분·복약·스트레스 요인 같은 임상 신호를 정리하고, 환자가 선택한 공유 범위 안에서 **진료 전 요약 리포트**로 전달하는 정신건강 AI 데모 플랫폼입니다.

> ⚠️ 데모/연구용. 진단·치료가 아닌 **기록·요약·전달** 도구. 의학적 판단은 의사가 합니다.
> 데모 데이터는 모두 **가상 환자**입니다.

<sub>AI Reboot Academy 21팀 MVP에서 시작한 데모 프로젝트.</sub>

## 라이브 데모
- 제품 소개 · 무로그인 인터랙티브 데모: <https://mindhub.forblune.com>
- 환자 앱(로그인 필요): `/app.html` · 의사 읽기전용 데모: `/doctor.html?demo=1`
...
- 커밋된 Supabase 키는 **공개용 anon 키**(RLS가 접근을 제어). Upstage·OpenAI·`service_role` 등 실제 비밀 키는 **환경변수에만** 두며 저장소에 넣지 않음.
